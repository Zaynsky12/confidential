...
async function main() {
  ...
  for (const pair of PAIRS) {
    ...
    try {
      ...
      if (existing.active) {
        console.log(`${pair.name} is already active.`);
        continue;
      }
      ...
      const pythIdBytes32 = pair.pythId.startsWith('0x') ? pair.pythId : '0x' + pair.pythId;

      const tx = await coreContract.addPair(
        pair.name, // string calldata pairName
        pythIdBytes32, // bytes32 pythFeedId
        pair.maxLev, // uint256 maxLeverage
        maxLongOI, // uint256 maxLongOI_
        maxShortOI, // uint256 maxShortOI_
        maxPositionPct // uint256 maxPositionPct
      );
      console.log(`Adding ${pair.name}... tx: ${tx.hash}`);
      await tx.wait();
      console.log(`${pair.name} added!`);
    } catch (e) {
      console.error(`Failed to add ${pair.name}:`, e);
    }
  }
