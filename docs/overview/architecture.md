...
| **2. ConfidentialTrading.sol** | **(Execution Engine)**<br>The primary interaction point for Traders. Handles order requests via a 2-step execution model (Request -> Keeper Execution), validates margin sufficiency, leverage, Pyth Oracle prices, and calculates Price Impact penalties before triggering logic within the Vault. |
